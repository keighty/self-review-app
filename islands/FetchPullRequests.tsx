    ...
    <div class="p-4 bg-white shadow-md rounded-lg">
      {token && (
        <p class="mb-4 text-gray-700">
          Your GitHub token will be used to fetch pull request data within the specified time range.
        </p>
      )}
      <form onSubmit={handleSubmit} class="space-y-4">
        <div>
          <label for="beginDate" class="block font-medium text-gray-700">Begin Date</label>
          <input
            id="beginDate"
            type="date"
      ...
      {prCount !== null && !error && (
        <p class="mt-4 text-green-700">Fetched {prCount} merged pull requests.</p>
      )}
    </div>
  );
}
